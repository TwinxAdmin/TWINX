...
A "why" egy mondatban indokolja, miért illik ez a beszállító a megadott igényhez. A "season" RÖVIDEN (max 1 mondat) jelezze, ha a keresett alapanyag épp nem szezonális, és mikor a legjobb beszerezni. A "market" LEGYEN RÖVID (max 1-2 mondat): csak azt mondd meg, hol tájékozódhat a partner az aktuális piaci árakról (pl. Budapesti Nagybani Piac heti árjegyzése, AKI PÁIR) — ne írj hosszú elemzést. A "tips" 2-3 rövid, gyakorlatias tárgyalási tanács. Az "outreach" egy kész, udvarias magyar megkereső üzenet, amit a partner kimásolhat és elküldhet a beszállítónak — hivatkozzon a keresett alapanyagra és a mennyiségre.`,
};
...
export const SUPPLIER_EU_DEFAULT_SEGMENTS = {
  intro: `Te egy nemzetközi, EU-n belüli beszerzési szakértő vagy, aki MAGYARORSZÁGI éttermeknek segít külföldi (EU-s) alapanyag-beszállítókat felkutatni. Valós, ellenőrizhető forrásokból dolgozz: konkrét, LÉTEZŐ EU-s gyártókat / termelőket / exportőröket keress, és — ahol van — a márka MAGYARORSZÁGI importőrét / disztribútorát is. SOHA ne találj ki céget, telefonszámot vagy e-mailt — amit nem találsz, hagyd üresen. Minden találathoz adj forrás-URL-t. A kereséshez a keresett terméket és a szűrőket FORDÍTSD LE a célország nyelvére és angolra, és a HELYI szakkifejezésekkel keress (a helyi termelők oldalai az adott ország nyelvén vannak). Fontos kontextus: a vevő magyar étterem, tehát az EU-n belüli beszerzés szabályai érvényesek — nincs vám, de kell közösségi adószám és áfa-kezelés (fordított adózás), nagyobb volumennél Intrastat; és kulcskérdés a szállítás/logisztika, a szállítási idő, valamint a jellemzően MAGASABB minimum rendelési mennyiség.`,
  task: `Válaszolj KIZÁRÓLAG érvényes JSON-nal, magyarázó szöveg nélkül, ebben a szerkezetben:
...
A "location" tartalmazza az országot (és régiót/várost). A "why" egy mondatban mondja meg, miért illik (pl. közvetlen gyártó, vagy van magyar disztribútora). A "season" RÖVIDEN (max 1 mondat) a termék elérhetőségéről / szállítási ütemről szóljon. A "market" LEGYEN RÖVID (max 1-2 mondat): a jellemző minimum rendelés vagy a szállítási idő egy tömör támpontja — NE írj hosszú piaci elemzést. A "tips" 2-3 rövid, gyakorlati tanács kifejezetten az EU-importhoz (logisztika, közösségi adószám / fordított áfa, nyelv, minta kérése). KÉT nyelven készíts kész, udvarias megkereső üzenetet ugyanazzal a tartalommal: az "outreach_en" ANGOL nyelvű (ezt küldi el a partner a külföldi beszállítónak; hivatkozzon a keresett termékre, a mennyiségre és arra, hogy magyar étterem közösségi adószámmal), az "outreach_hu" pedig ugyanennek a MAGYAR fordítása (hogy a partner pontosan lássa, mit küld). Ahol csak lehet, a közvetlen külföldi gyártó mellé javasolj egy MAGYAR importőrt/disztribútort is alternatívaként.`,
};
...
export function composeSupplierPromptEu(
  ...
  const lines = [
    ...
    // A találatok minősége azon múlik, hogy HELYI nyelven keres-e a modell.
    `NAGYON FONTOS: a keresett terméket és a szűrőket fordítsd le a célország nyelvére ÉS angolra, és ezekkel a HELYI szakkifejezésekkel keress (pl. „liszt" → olaszul „farina", angolul „flour"; „beszállító" → „fornitore"). Vedd figyelembe a helyi termékkategóriákat és minőségi jelöléseket is (pl. pizzához „farina tipo 00"). A helyi termelők weboldalai és katalógusai jellemzően az adott ország nyelvén vannak — ott keress elsősorban.`,

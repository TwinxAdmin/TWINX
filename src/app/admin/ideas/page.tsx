...
function IdeaRow({ idea }: { idea: Idea }) {
  return (
    <li className="border border-gray-200 p-3 text-sm">
  ...
  return (
    <main className="mx-auto max-w-3xl space-y-6 p-6 font-sans">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-semibold">Admin — Ötletláda</h1>
      ...
      <section>
        <h2 className="font-medium">Jóváhagyásra vár ({pending.length})</h2>
        {pending.length === 0 ? (
          <p className="mt-2 text-sm text-gray-500">Nincs függőben lévő ötlet.</p>
        ) : (
      ...
      <section>
        <h2 className="font-medium">Elbírált ötletek ({others.length})</h2>
        {others.length === 0 ? (
          <p className="mt-2 text-sm text-gray-500">Még nincs elbírált ötlet.</p>
        ) : (
          <ul className="mt-2 space-y-2">
            {others.map((idea) => (
              <IdeaRow key={idea.id} idea={idea} />
